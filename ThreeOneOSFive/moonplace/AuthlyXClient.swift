...
enum AuthlyXClient {
    private static var apiBase: URL {
        // Usa la URL de MoonConfig (si la tienes definida)
        // Si no, usa la URL por defecto de AuthlyX
    ...
    static func register(sessionID: String, username: String, password: String, license: String) async throws -> Response {
        try await post([
            "type": "register",
            "name": MoonConfig.authlyxAppName,
            "ownerid": MoonConfig.authlyxOwnerId,
    ...
    static func login(sessionID: String, username: String, password: String) async throws -> Response {
        try await post([
            "type": "login",
            "name": MoonConfig.authlyxAppName,
            "ownerid": MoonConfig.authlyxOwnerId,
